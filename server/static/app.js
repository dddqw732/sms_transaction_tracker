...
async function bootstrapAuth() {
    ...
    try {
        ...
        showView('dashboard');
        initDashboard();
        if (typeof updateBusinessUI === 'function') updateBusinessUI(currentCompany);
    } catch {
        if (accessToken) {
            localStorage.removeItem('cashin_access_token');
            accessToken = '';
...
function bindAuthUI() {
    ...
    signupForm.addEventListener('submit', async (event) => {
        ...
        const selectedBtRadio = document.querySelector('input[name="signup-business-type"]:checked');
        const business_type = selectedBtRadio ? selectedBtRadio.value : 'restaurant';
...
function createTransactionCard(txn, isLatest = false) {
    ...
    const catBadge = txn.category ? `<span class="txn-category-badge" style="background:rgba(99,102,241,0.1);color:#6366f1;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:700;margin-left:6px;">🏷️ ${txn.category}</span>` : '';
    ...
    return `
        <div class="txn-card${latestClass}" onclick="showTxnDetails(${txn.id})">
            ...
            <div class="txn-amount-col" style="display:flex;align-items:center;gap:10px;">
                <div class="txn-amount ${typeClass}">${amountSign} ${formatAmount(txn.amount, txn.currency)}</div>
                <button class="btn btn-secondary btn-xs" style="padding:4px 8px;font-size:11px;font-weight:700;border-radius:6px;white-space:nowrap;" onclick="event.stopPropagation(); if (typeof openClassifyModal === 'function') openClassifyModal(window.allTransactions.find(t=>t.id===${txn.id}))" title="Classify Transaction">
                    ${txn.category ? '✏️ ' + txn.category : '⚡ Classify'}
                </button>
            </div>
        </div>
    `;
...
function updateMetrics() {
    ...
    allTransactions.forEach((t) => {
        ...
        if (t.type === 'Received') receivedUSD += amt;
        else sentUSD += amt;
    });

    totalReceivedEl.textContent = formatAmount(receivedUSD, 'USD');
    totalSentEl.textContent = formatAmount(sentUSD, 'USD');

    const net = receivedUSD - sentUSD;
    netEl.textContent = `${net >= 0 ? '+' : '-'} ${formatAmount(Math.abs(net), 'USD')}`;
    netEl.className = `metric-value ${net >= 0 ? 'text-success' : 'text-danger'}`;

    if (typeof window.renderCurrencyBalances === 'function') {
        window.renderCurrencyBalances();
    }

    if (typeof window.renderDashboardProviderCards === 'function') {
        window.renderDashboardProviderCards();
    }
}
...
function applyFilters() {
    ...
    filteredTransactions = allTransactions.filter((txn) => {
        const matchesSearch =
            ...
            (txn.receiver && txn.receiver.toLowerCase().includes(query)) ||
            (txn.category && txn.category.toLowerCase().includes(query)) ||
            (txn.provider && txn.provider.toLowerCase().includes(query)) ||
            (txn.transaction_id && String(txn.transaction_id).toLowerCase().includes(query)) ||
            (txn.timestamp && txn.timestamp.toLowerCase().includes(query));
        const matchesType = filterType === '' || txn.type === filterType;
        const matchesProvider = filterProvider === '' || txn.provider === filterProvider;
        const matchesHighValue = !window.highValueOnly || Number(txn.amount || 0) >= 100;
        return matchesSearch && matchesType && matchesProvider && matchesHighValue;
    });
...
window.setSearchQuery = (q) => { searchQuery = q; applyFilters(); };
window.setFilterType = (t) => { filterType = t; applyFilters(); };
window.setHighValueFilter = (threshold) => { window.highValueOnly = !window.highValueOnly; applyFilters(); };
...
function renderReport() {
    ...
    if (keys.length === 0) {
        if (reportChart) reportChart.innerHTML = '<div class="empty-state"><p>No transactions to display.</p></div>';
        if (summaryContent) summaryContent.innerHTML = '';
        if (totalReceivedEl) totalReceivedEl.textContent = '0.00 SLSH';
        if (totalSentEl) totalSentEl.textContent = '0.00 SLSH';
        if (netEl) {
            netEl.textContent = '0.00 SLSH';
            netEl.className = 'metric-value';
        }
        if (comparisonSummary) comparisonSummary.classList.add('hidden');
        return;
    }
    ...
    let totalSent = 0;
    keys.forEach((key) => {
        totalReceived += groups[key].received;
        totalSent += groups[key].sent;
    });

    if (totalReceivedEl) totalReceivedEl.textContent = formatAmount(totalReceived, 'SLSH');
    if (totalSentEl) totalSentEl.textContent = formatAmount(totalSent, 'SLSH');
...
function showToast(txnOrMessage, toastType) {
    const container = document.getElementById('toast-container');
    if (!container) return;

    // Support simple string toasts (e.g. from new_features.js)
    if (typeof txnOrMessage === 'string') {
        const toast = document.createElement('div');
        toast.className = `toast ${toastType === 'success' ? 'received' : 'sent'}`;
        toast.innerHTML = `
            <div class="toast-body">
                <div class="toast-title" style="font-size:14px;">${txnOrMessage}</div>
            </div>
...
async function showTxnDetails(txnId) {
    ...
    let allocationsHtml = '<p class="text-muted" style="font-size:12px;">No split allocations recorded.</p>';
    try {
        const res = await apiFetch(`/api/transactions/${txnId}/allocations`);
        if (res.ok) {
            const allocs = await res.json();
            if (allocs && allocs.length > 0) {
                allocationsHtml = allocs.map(a => `
                    <div style="background:#f8fafc;padding:8px 12px;border-radius:8px;margin-top:6px;font-size:12px;border:1px solid #e2e8f0;">
                        <div style="display:flex;justify-content:space-between;font-weight:700;">
                            <span>${a.item_name || a.allocation_type} ${a.item_quantity ? `(${a.item_quantity}x)` : ''}</span>
                            <span style="color:#6366f1;">${a.converted_amount} ${a.target_currency}</span>
                        </div>
                        <div style="color:#64748b;margin-top:2px;">
                            Delivery: <b>${a.delivery_method}</b> • Original: ${a.original_allocated_amount} ${txn.currency}
                        </div>
                    </div>
                `).join('');
            }
...
function handleNewTransaction(txn) {
    ...
    // Show popup with 1-tap classify button
    const popup = document.getElementById('notification-popup');
    if (popup) {
        popup.dataset.txn = JSON.stringify(txn);
        const body = document.getElementById('notification-body');
        if (body) {
            body.innerHTML = `
                <div style="font-size:16px;font-weight:800;color:#10b981;">+ ${txn.currency === 'USD' ? '$' : ''}${Number(txn.amount).toLocaleString()} ${txn.currency !== 'USD' ? txn.currency : ''}</div>
                <div style="font-size:13px;color:#334155;margin-top:2px;">From: <b>${txn.sender || 'Customer'}</b> via <b>${txn.provider || 'Payment SMS'}</b></div>
            `;
        }
...
function bindDashboardEvents() {
    ...
    navBtns.forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (targetId === 'tab-invoices' && typeof loadInvoices === 'function') loadInvoices();
            if (targetId === 'tab-catalog' && typeof loadCatalogItems === 'function') loadCatalogItems();
            if (targetId === 'tab-summary' && typeof loadAnalyticsReport === 'function') loadAnalyticsReport(typeof currentReportRange !== 'undefined' ? currentReportRange : 'today');
            if (targetId === 'tab-dashboard' && typeof renderDashboardProviderCards === 'function') renderDashboardProviderCards();
        });
    });
    ...
    if (categoryFilter) {
        categoryFilter.addEventListener('change', (event) => {
            // Store and apply category filter
            const val = event.target.value;
            filteredTransactions = allTransactions.filter(txn => {
                const matchesCat = val === '' || (txn.category || '') === val;
                const matchesType = filterType === '' || txn.type === filterType;
                const matchesProv = filterProvider === '' || txn.provider === filterProvider;
                const q = searchQuery.toLowerCase();
                const matchesSearch = !q ||
                    (txn.sender && txn.sender.toLowerCase().includes(q)) ||
                    (txn.category && txn.category.toLowerCase().includes(q)) ||
                    (txn.transaction_id && String(txn.transaction_id).toLowerCase().includes(q));
                return matchesCat && matchesType && matchesProv && matchesSearch;
            });
            renderLedger();
        });
    }
    ...
    if (generateReportBtn && reportTypeSelect && compareTypeSelect) {
